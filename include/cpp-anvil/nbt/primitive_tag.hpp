...
public:
    ...
    explicit PrimitiveTag(const std::string& name)
        requires(!detail::IsStringType<T>)
        : BasicTag(name) { };
...
protected:
    virtual bool isEqual(const BasicTag& other) const override
    {
        ...
        return BasicTag::isEqual(other) && m_value == otherTag.m_value;
    }
...
inline bool FloatTag::isEqual(const BasicTag& other) const
{
    ...
    return BasicTag::isEqual(other) && detail::almostEqual<FloatType>(m_value, otherTag.m_value);
}
...
inline bool DoubleTag::isEqual(const BasicTag& other) const
{
    ...
    return BasicTag::isEqual(other) && detail::almostEqual<DoubleType>(m_value, otherTag.m_value);
}
